...
def init_app(app):
    ...
    # НОВАЯ КОМАНДА: Обновление изображений категорий
    @app.cli.command('update-category-images')
    def update_category_images():
        ...
        with app.app_context():
            ...
            click.echo('Начинаю обновление изображений категорий...')
            updated = update_all_category_images()
            
            if updated > 0:
                click.echo(f'✅ Обновлено {updated} изображений категорий')
            else:
                click.echo('⚠️ Не удалось обновить изображения категорий или нечего обновлять')
